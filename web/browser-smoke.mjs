...
async function expectText(page, text, name) {
  await page.getByText(text, { exact: false }).filter({ visible: true }).first().waitFor({ timeout: 20000 })
...
try {
  ...
  await page.locator('.el-table__body-wrapper tbody tr')
    .filter({ hasText: reviewOrderNo })
    .first()
    .click()
  await expectText(page, '售后处理台', 'admin after-sale review detail visible')
  ...
  await expectText(page, 'SLA 跟进', 'admin SLA center page visible')
  await page.getByPlaceholder('售后单号、订单号或商品名').fill(reviewOrderNo)
  await page.getByRole('button', { name: '查询' }).first().click()
  ...
  await expectText(page, '售后处理台', 'admin SLA task jumps to review desk')
  ...
  await page.getByPlaceholder('售后单号、订单号或商品名').fill(reviewOrderNo)
  await page.getByRole('button', { name: '查询' }).first().click()
  await expectText(page, reviewOrderNo, 'admin after-sale review target after image evidence')
  await page.locator('.el-table__body-wrapper tbody tr')
    .filter({ hasText: reviewOrderNo })
    .first()
    .click()
  ...
  await page.getByPlaceholder('售后单号、订单号或商品名').fill(reviewOrderNo)
  await page.getByRole('button', { name: '查询' }).first().click()
  await expectText(page, reviewOrderNo, 'admin after-sale review target after risk visible')
  await page.locator('.el-table__body-wrapper tbody tr')
    .filter({ hasText: reviewOrderNo })
    .first()
    .click()
  await expectText(page, '售后处理台', 'admin after-sale review detail after risk visible')
  ...
  await expectText(page, '售后运营首页', 'operations home visible')
  await expectText(page, '退换货客服处理台', 'operations home title visible')
  await expectText(page, '客服处理流', 'operations workflow visible')
  await expectText(page, '今日关注', 'operations focus visible')
  record('operations home removes defense wording', !(await page.getByText('答辩', { exact: false }).count()), '答辩 absent')
  record('operations home removes demo wording', !(await page.getByText('演示', { exact: false }).count()), '演示 absent')
  ...
  await expectText(page, '系统状态', 'dashboard page visible')
  const systemStatus = await apiGet('/system/status')
  const currentModelName = systemStatus?.ai?.modelName || systemStatus?.ai?.selectedModelName || 'local-rule-template'
  await expectText(page, 'AI 接入信息', 'dashboard AI status panel visible')
  await expectText(page, currentModelName, 'dashboard current model visible')
  ...
  await expectText(page, 'AI 质检', 'ai test page visible')
  ...
  await expectText(page, currentModelName, 'ai test current model visible')
  ...
  await page.getByRole('tab', { name: 'AI 建议' }).click()
  await expectText(page, '处理建议', 'chat decision summary visible')
  await expectText(page, '业务工具', 'chat business tools visible')
  await expectText(page, '建议追问', 'chat suggested questions visible')
  await page.getByRole('tab', { name: '工单与流程' }).click()
  await expectText(page, '回答过程', 'chat process flow visible')
  await page.getByRole('tab', { name: '图片/凭证风险' }).click()
  await expectText(page, '图片真实性预审', 'chat image risk tab visible')
  ...
  const chatTicketMarker = `浏览器图片工单 ${Date.now()}`
  ...
  await expectText(page, '服务日志', 'logs page visible')
