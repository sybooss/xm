...
- 订单与售后：内置订单样例，支持订单查询、订单维护、售后记录维护和订单上下文融合。
...
- 登录权限：提供演示管理员登录和客户注册，并对新增、修改、删除类操作进行角色拦截。
...
- 默认演示管理员为 `admin / 123456`，可通过 `APP_AUTH_ADMIN_PASSWORD` 修改密码；注册入口会创建客户角色账号。
