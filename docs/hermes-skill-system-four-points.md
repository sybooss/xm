...
每张截图下方均从业务角色、页面操作、流程位置和答辩价值四个角度说明，重点展示顾客申请、后台审核、AI 增强、规则维护、人工升级和日志追踪等核心功能的运行效果。
...
本章说明的 Skill 是 `returns-assistant-ops`。它面向本项目的运营统计与业务复盘，用来把订单、售后申请、工单、知识库、AI 调用日志和处理轨迹组织成可阅读的运营摘要。通过该 Skill，Hermes 可以围绕“今日售后运营情况”“SLA 风险与工单积压情况”“AI 客服运行效果”等问题输出统计结果和处理建议。
...
- 指出可能导致退货的商品、原因或意图。
- 说明哪些页面、表和日志可以作为答辩证据。
- 数据少时说明是演示数据，表为空时标注“暂无数据”。
- 所有统计结论以数据库和日志为依据。

```

`returns-assistant-ops` 覆盖订单概览、售后申请、退货率/售后率、退款/换货/投诉分布、工单积压、SLA 风险、AI 调用质量、知识库命中、凭证审核、处理日志和客户评价。它的价值不只体现在工具名称上，更体现在系统的运营复盘能力上：页面展示说明功能已经实现，接口调用说明链路已经打通，而运营统计进一步回答系统运行情况、紧急业务问题、AI 稳定性、知识库命中情况和售后效率等问题。

## 四、系统结合对应 Skill 完成相关功能

系统结合 `returns-assistant-ops` 后，可以把业务统计演示成一条清晰链路。用户提出“今日售后运营情况”“当前退货率与售后率”“AI 客服运行效果”“SLA 风险与工单积压情况”等问题，Skill 会把问题识别为本项目的业务统计请求，并运行统计脚本读取本地演示数据库，聚合订单、售后申请、工单、AI 调用、知识检索、凭证审核、流程日志和服务评价，然后输出中文运营摘要。

| 用户问题 | Skill 执行动作 | 系统数据来源 | 输出结果 |
| --- | --- | --- | --- |
| 今日售后运营情况 | 生成今日业务快照。 | 订单表、售后申请表、售后处理日志。 | 今日订单、售后数量、售后率、退款金额和风险摘要。 |
| 当前退货率与售后率 | 按同一时间范围统计售后申请与订单数量。 | 订单数据、售后申请数据。 | 退货、换货、退款、投诉分布及占比。 |
| AI 客服运行效果 | 统计 AI 调用成功、失败、跳过和平均耗时。 | AI 调用日志、聊天与处理轨迹。 | AI 成功率、延迟情况和兜底风险。 |
| SLA 风险与工单积压 | 统计未关闭、高优先级、紧急工单。 | 工单数据和 SLA 相关数据。 | 客服优先处理建议。 |
