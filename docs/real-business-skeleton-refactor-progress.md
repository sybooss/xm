...
| 优先级 | 候选项 | 原因 | 建议版本 |
| --- | --- | --- | --- |
| P0 | 自动化验证硬化 | 当前已有主链路、角色路由和部分接口权限覆盖，下一步应把脚本拆得更稳定并减少对长链路顺序的依赖 | V7.0 |
| P0 | 交付脚本和启动自检收口 | 当前验证需要手动确认后端、前端、MySQL、sub2api 均可用，真实交付应有一键自检和明确失败提示 | V7.0 |
| P1 | 顾客端处理结果说明 | 评价已可提交，但顾客端还没有独立的客服最终回复或处理结果说明视图 | V7.0 |
| P1 | 客户画像统计深化 | 当前画像已经汇总订单、售后、工单和评价，但时间窗口、低分原因分布和复购指标仍偏轻量 | V7.0 |
