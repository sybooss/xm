...
| V6.0 | 已完成 | 2026-05-06 | 双端默认入口、路由守卫、菜单隔离、接口角色校验和权限冒烟覆盖 | 后端编译、前端构建、全链路冒烟、主浏览器测试和角色浏览器测试均通过 | 双端边界从“页面习惯”收口为前后端共同约束，下一步应硬化自动化验证和交付脚本 |
| V7.0 | 已完成 | 2026-05-06 | 交付预检脚本、一键验证前置检查、README 和测试清单同步 | 交付预检、后端编译、前端构建、全链路冒烟、主浏览器测试和角色浏览器测试均通过 | 交付前检查从“记命令”升级为可执行预检，下一步应做最终审视和可优化项清零 |
| V8.0 | 已完成 | 2026-05-06 | 顾客处理结果说明、客户画像统计深化、最终审视和停止优化说明 | 后端编译、前端构建、全链路冒烟、主浏览器测试和角色浏览器测试均通过 | V0.1-V8.0 主线闭环完成，P0/P1 已清零，剩余优化均为后续扩展型 P2/P3 |
...
| 优先级 | 候选项 | 原因 | 建议版本 |
| --- | --- | --- | --- |
| 已处理 | 最终审视和可优化项清零 | V8.0 已完成终局审计、停止前检查清单和停止优化说明 | V8.0 |
| 已处理 | 顾客端处理结果说明 | V8.0 已在顾客售后详情中新增处理结果说明、客服最终回复和下一步动作 | V8.0 |
| 已处理 | 客户画像统计深化 | V8.0 已补近 30 天售后、投诉占比、低分评价、重复售后、低分原因和运营建议 | V8.0 |
...
- 完成日期：2026-05-06
- 本版改动文件：更新 `AuthService`/`AuthServiceImpl`，统一新增 `requireAdmin` 和 `requireCustomer`；更新 `AuthInterceptor`、`CustomerAfterSaleController`、`ServiceReviewController`、`ServiceReviewService`、`ServiceReviewServiceImpl`、`ServiceTicketController`；更新顾客售后中心、前端路由、登录页、`TicketPanel`、`browser-smoke.mjs`、`browser-role-smoke.mjs` 和 `tools/full-smoke-test.ps1`。
- 验证命令和结果：`mvn -q -DskipTests package` 通过；首次编译因旧后端 jar 被运行进程占用而失败，停止 `returns-assistant-0.0.1-SNAPSHOT.jar` 进程后重跑通过；`npm run build` 通过；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\full-smoke-test.ps1` 通过且 `FAILED_COUNT=0`；`npm run test:browser` 通过且 `FAILED_COUNT=0`；`npm run test:browser:roles` 通过且 `FAILED_COUNT=0`。
- 业务价值：顾客登录默认进入“我的售后”，管理员登录默认进入“售后审核工作台”；答辩展示保留为管理端菜单，但不再作为真实业务默认首页；顾客售后中心支持 `?focus=售后ID` 直达详情，后续通知和评价链接可以直接打开指定售后单。
- 双端真实度审视：菜单和路由守卫现在同时验证顾客只能看到“我的售后/在线咨询”，管理员只能看到管理菜单；顾客访问管理路由会回到顾客售后中心，管理员误入顾客售后路由会回到审核工作台；主浏览器脚本覆盖顾客申请、管理员审核、SLA、工单、AI 草稿、完成、评价和客户画像，角色脚本覆盖双端菜单与路由隔离。
- 状态机和权限审视：管理端 `@OperatorAnno` 统一复用 `requireAdmin`；顾客售后、补凭证、评价提交和评价查询统一要求 CUSTOMER 角色；评价查询新增售后单归属校验；会话下手动创建工单改为管理员动作，顾客端按钮隐藏，避免客户绕过客服转接规则直接创建人工工单。
- AI 辅助边界审视：本版不新增 AI 自动处置；AI 草稿、风险和摘要仍在管理员确认链路内，权限加固不会让 AI 获得越权修改状态或创建业务单据的能力。
- 遗留问题：顾客端仍缺独立的客服最终回复/处理结果说明；全链路验证脚本较长，对后端、前端、MySQL 和 sub2api 的运行状态有依赖；客户画像统计仍可以深化为时间窗口和低分原因分析。
- 下一轮优化：进入 V7.0，优先硬化自动化验证和交付启动自检，再考虑顾客端结果说明与画像统计深化。
- Git 提交：`feat: harden dual-end navigation permissions`。

## V7.0 审视记录

- 完成日期：2026-05-06
- 本版改动文件：新增 `tools/delivery-preflight.ps1`，更新 `tools/verify-program.ps1`、`README.md`、`docs/test-cases.md` 和本进度文档。
- 验证命令和结果：`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\delivery-preflight.ps1` 通过且 `FAILED_COUNT=0`；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\delivery-preflight.ps1 -RequireRunningServices -RequireMysql -RequireSub2api` 通过且 `FAILED_COUNT=0`；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-program.ps1` 通过且 `FAILED_COUNT=0`；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-program.ps1 -WithSmoke -WithBrowser -WithRoleBrowser` 通过且 `FAILED_COUNT=0`。
- 业务价值：交付前可以先检查关键文件、Java/Maven/Node/NPM/Git、`.env`/`output`/`tmp`/`web/dist`/`server/target` 等忽略规则、禁止提交产物、可选 MySQL、后端、前端和 sub2api 状态，减少“环境没准备好但后续脚本才爆错”的情况。
- 双端真实度审视：README 和测试清单已把默认入口改为顾客“我的售后”和管理员“售后审核工作台”，不再把答辩展示中心写成真实业务默认首页。
- 状态机和权限审视：本版不改业务状态机，但交付预检和角色浏览器脚本一起成为权限回归前置门禁，后续修改入口、菜单或接口时更容易及时发现越权问题。
- AI 辅助边界审视：预检支持把 sub2api 作为可选或必选项检查，便于区分 AI 网关未启动和主业务不可用；AI 不可用时主业务仍可通过本地兜底演示。
- 遗留问题：顾客端仍缺独立的客服最终回复/处理结果说明；客户画像统计仍可继续深化；长链路浏览器脚本已经更稳定，但后续还可以拆分为更小的分层测试。
- 下一轮优化：进入 V8.0，做最终审视和可优化项清零，决定剩余 P1/P2 是否继续实现或明确取舍。
- Git 提交：`test: add delivery preflight checks`。

## V8.0 审视记录

- 完成日期：2026-05-06
- 本版改动文件：更新 `AfterSaleApplication`、`AfterSaleApplicationServiceImpl`，为顾客端售后详情补充 `customerResultSummary`、`customerFinalReply` 和 `customerNextAction`；更新 `CustomerProfile`、`DemoOrderMapper`、`AfterSaleApplicationMapper`、`ServiceReviewMapper` 及 XML，补齐画像统计；更新 `CustomerAfterSaleCenterView.vue`、`AdminCustomerProfileView.vue`、`tools/full-smoke-test.ps1`、`web/browser-smoke.mjs`、`docs/test-cases.md`、`docs/real-business-skeleton-refactor-plan.md` 和本进度文档。
- 验证命令和结果：首次 `mvn -q -DskipTests package` 因旧运行 jar 占用重命名失败，停止 `returns-assistant-0.0.1-SNAPSHOT.jar` 进程后重跑通过；`npm run build` 通过；`powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\verify-program.ps1 -WithSmoke -WithBrowser -WithRoleBrowser` 通过且 `FAILED_COUNT=0`，覆盖交付预检、后端编译、前端构建、全链路接口冒烟、顾客/管理员浏览器主流程和角色路由隔离。
- 业务价值：顾客端现在能在售后详情中直接看到“处理结果说明”、客服最终回复和下一步动作，不再只能从时间线倒推处理结论；管理员客户画像新增近 30 天售后、投诉占比、低分评价、重复售后、低分原因和运营建议，画像从总量展示提升为可行动的运营判断。
- 双端真实度审视：顾客端围绕“我这单处理到哪里、客服最终怎么说明、我下一步做什么”闭合；管理员端围绕“这个客户最近是否频繁售后、投诉占比如何、低分原因是什么、该怎么跟进”闭合。两端仍共享同一售后主对象和处理日志，视角不同但证据一致。
- 状态机和权限审视：V8 不新增绕过状态机的动作，处理结果说明只从既有售后状态、审核/补材料/完成日志和已采纳草稿日志推导；画像统计仍由管理员画像接口返回，顾客不能访问他人画像或后台聚合。状态流转、评价归属、角色菜单和越权路由继续由全量验证覆盖。
- AI 辅助边界审视：客服最终回复来自管理员采纳过的回复草稿日志，AI 仍只生成草稿，不直接发给顾客、不直接修改售后状态；AI 不可用时草稿仍可由本地模板生成，主业务闭环不依赖 AI 成功。
- 遗留问题：没有继续实现真实图片文件上传、多角色细分到仓库/财务、定时 SLA 通知、真实支付退款回调、画像图表化和测试脚本分层拆分。这些都属于后续产品扩展或工程规模化，不影响“从客服演示到真实双端售后系统”的主目标达成，按 P2/P3 处理。
- 下一轮优化：当前主线无 P0/P1 高价值缺口，可以停止本轮改造。后续如继续扩展，建议另立目标从“真实文件上传/仓库财务角色/支付回调/通知系统/测试分层”中选择一条独立推进。
- Git 提交：`feat: finalize dual-end after-sale workflow`。

## 5. 停止优化说明

截至 V8.0，本轮目标已经从“AI 客服演示器”收口为可验证的真实双端售后系统：顾客端可独立查看订单、提交售后、补充凭证、查看进度、理解处理结果并评价；管理员端可审核、驳回、要求补材料、创建关联工单、确认完成、查看 SLA 风险、采纳/废弃 AI 草稿并查看客户画像。AI 已被限制在副驾驶层，关闭或失败时主业务仍可运行。

最终审视后，进度文档中已无未处理 P0/P1。剩余项包括真实文件上传、仓库/财务细角色、定时通知、真实退款回调、画像图表化和测试脚本拆分，它们会让系统更接近企业级生产系统，但已经超出本轮“从演示到真实双端业务骨架”的必要边界，因此作为 P2/P3 后续扩展保留。
...
- [x] 顾客端完整闭环：订单、申请、凭证、进度、补充、处理结果说明、确认、评价。
- [x] 管理员端完整闭环：审核、驳回、补材料、分派、处理、SLA、客户历史和画像统计。
- [x] AI 可关闭，关闭后主业务流程仍可跑通，AI 只保留为草稿和建议层。
- [x] 状态机禁止非法流转，并记录每次变更。
- [x] 证据链完整串联订单、售后、工单、凭证、AI 草稿、评价、日志。
- [x] 自动化验证覆盖主流程，并通过 `verify-program.ps1 -WithSmoke -WithBrowser -WithRoleBrowser` 全量门禁。
- [x] 进度文档无 P0/P1 未处理问题；剩余 P2/P3 已写入停止优化说明。
