...
| `/` | `ShowcaseView` | 默认跳转到答辩展示中心 |
| `/showcase` | `ShowcaseView` | 答辩展示中心，集中呈现演示顺序、8 个已实现亮点、现场检查清单和关键页面入口 |
| `/dashboard` | `DashboardView` | 系统总览、数据库状态、AI 状态、快速入口 |
...
- 顶部展示项目定位：电商退换货 AI 客服系统。
- 实时读取 `/system/status`，展示数据库状态、AI 状态、当前模型和本地兜底策略。
- 按答辩顺序组织五个演示步骤：登录与权限、咨询工作台、知识库依据、人工工单、日志与测试。
- 汇总 8 个真实已实现亮点：AI 流式客服、RAG 知识增强、多轮上下文承接、LangChain4j 工具调用、智能工单升级、本地规则兜底、权限与安全控制、日志可追溯。
- 右侧提供页面跳转入口和现场检查清单，避免答辩时在多个页面之间迷路。

设计要求：

- 风格保持白底、克制、高级产品感，避免普通后台模板的廉价感。
- 所有亮点必须对应真实页面、接口、日志或代码，不展示不存在的功能。
- 普通客户角色不能访问该页面，客户访问时会被路由守卫重定向回 `/chat`。
